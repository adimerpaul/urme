  ...
  <q-page class="q-pa-sm bg-grey-2">
    <q-card flat bordered class="q-mb-sm">
      <q-card-section class="row items-center q-col-gutter-sm">
        <div class="col">
          <div class="text-h6 text-weight-bold">Configuración Hematología</div>
          <div class="text-caption text-grey-7">
            Controla qué variables se muestran en la captura de hematología según las prestaciones de la solicitud
          </div>
        </div>
        <div class="col-auto">
...
<script>
export default {
  name: 'ConfiguracionHematologiaPage',
  data () {
    return {
      loading: false,
      filter: '',
  ...
  methods: {
    load () {
      this.loading = true
      this.$axios.get('datos-hematologia')
        .then(res => {
    ...
    save (dato) {
      dato.saving = true
      this.$axios.put(`datos-hematologia/${dato.id}`, {
        activo: dato.activo,
